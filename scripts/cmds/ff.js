...
module.exports.config = {
  ...
  version: "1.0.0",
  hasPermission: 0,
  credits: "Mây Trắng, Lemon (FIX and RMK)", 
...
const originalCredits = "Mây Trắng, Lemon (FIX and RMK)";
module.exports.run = async function({ api, event, args }) {
  ...
  const ffId = args[0];
  const apiUrl =`https://www.public.freefireinfo.site/api/info/sg/${ffId}?key=lemonee`;// thay api đi mấy em
  try {
    const response = await axios.get(apiUrl);
    const data = response.data;
    if (data) {
      let resultMessage = "⭓──────𝑻𝒉𝒐̂𝒏𝒈 𝑻𝒊𝒏 𝑻𝒂̀𝒊 𝑲𝒉𝒐𝒂̉𝒏──────⭓\n";
